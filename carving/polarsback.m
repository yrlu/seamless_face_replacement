function arr = polarsback(polar_arr, rmin, rmax)
% LOGSAMPLE  Compute reverse of log-polar transform of image
%     ARRAY = polarsback(polar_arr, RMIN, RMAX) takes an array of samples
%     on a logarithmic grid and resamples to a conventional grid. 
% 
%     polar_arr is an NW x NR log-sampled array, as returned by LOGSAMPLE.
...
% Edited by Dongni Wang 2016

[nw, nr, ~] = size(polar_arr);
t = fliptform(logtform(rmin, rmax, nr, nw));
xc = ceil(t.tdata.rmax) + 1;
X = 2*xc-1;
Xdata = [1, X] - xc;
Udata = [0, nr-1];
% polar_arr = padarray(polar_arr, [1 0], 'circular');  %wraps round in theta
Vdata = [-1, nw];       % [0 nw-1] before padding
Size = [X, X];
arr = imtransform(polar_arr, t, ...
